...
#ifndef NCK_GRAPHICS_GLX2_H
#define NCK_GRAPHICS_GLX2_H
...
*/
class Texture_GLX2:public virtual Texture
{
public:
    Texture_GLX2(Device_GLX2 *dev);
    virtual ~Texture_GLX2();
    ...
    ResourceProxy<Texture_GLX2> * GetProxyManager();
#endif
...
protected:
    ...
    ResourceProxy<Texture_GLX2> * m_Proxy;
#endif
};
 ...
 */
class Texture2D_GLX2:public virtual Texture2D, public Texture_GLX2
{
public:
    Texture2D_GLX2(Device_GLX2 *dev);
    static Texture2D_GLX2 * Load(Device_GLX2 *dev,const std::string & filename, bool mipmaps);
    virtual ~Texture2D_GLX2();
 ...
 */
class TextureCubemap_GLX2:public virtual TextureCubeMap, public Texture_GLX2
{
public:
    /**
    * Load cubemap from a collection of images. 
    */
    static TextureCubemap_GLX2 * Load(Device_GLX2 * dev, const std::vector<Core::Image*> & images, bool mipmaps);    

    /**
    * Load cubemap from a tar package.
    */
    static TextureCubemap_GLX2 * Load(Device_GLX2 * dev,const std::string & packageFilename, bool mipmaps);


    void Create(unsigned int width, unsigned int height, Format format, bool renderTarget);

    TextureCubemap_GLX2(Device_GLX2 *dev);
    virtual ~TextureCubemap_GLX2();
 ...
 */
class Texture3D_GLX2:public virtual Texture3D, public Texture_GLX2
{
public:
    /**
    * Load 3d texture from a tar package.
    */
    static Texture3D_GLX2 * Load(Device_GLX2 * dev,const std::string & packageFilename, bool mipmaps);

    /**
    * Load 3d texture from multiple filenames.
    */
    static Texture3D_GLX2 * Load(Device_GLX2 * dev,const std::vector<std::string> & filenames, bool mipmaps);

    /**
    * Load 3d texture from a collection of images.
    */
    static Texture3D_GLX2 * Load(Device_GLX2 * dev,const std::vector<Core::Image*> & images,bool mipmaps);

    Texture3D_GLX2(Device_GLX2 *dev);
    virtual ~Texture3D_GLX2();
 ...
 */
class RTManager_GLX2 : public RTManager
{
public:
    class AttachedTexture{
    public:
        AttachedTexture(Texture_GLX2* tex,
                uint32_t target,uint32_t face){
            this->texture = tex;
            this->target = target;
            this->face = face;
        }
        Texture_GLX2* texture;
        uint32_t target;
        uint32_t face;
    };

    RTManager_GLX2(Device_GLX2 *dev);
    virtual ~RTManager_GLX2();
 ...
 */
class Mesh_GLX2: public virtual Mesh
{
public:
    struct FacesGroup{
        uint32_t count;
 ...
 */
class Program_GLX2:public virtual Program
{
public:
    Program_GLX2(Device_GLX2 *dev);
    virtual ~Program_GLX2();
...
#ifdef NCK_GRAPH_RES_PROXY
    ResourceProxy<Program_GLX2> * GetProxyManager();
#endif
...
protected:
    ...
    std::map<std::string,Shader_GLX2_Variable*> m_Variables;
    GLuint m_Program;

#ifdef NCK_GRAPH_RES_PROXY
    ResourceProxy<Program_GLX2> * m_Proxy;
#endif
 ...
 */
class OcclusionQuery_GLX2: public OcclusionQuery
{
public:
    OcclusionQuery_GLX2(Device_GLX2 *dev);
    virtual ~OcclusionQuery_GLX2();
 ...
 */
class DynamicMesh_GLX2 : public virtual DynamicMesh{
public:
    DynamicMesh_GLX2(Device_GLX2 *dev);
    virtual ~DynamicMesh_GLX2();
 ...
 */
class Device_GLX2:public Device
{
public:
    Device_GLX2(Core::Window *wnd,
    ...
    Program_GLX2 *m_ActiveProgram;

    /// Active FBO.
    RTManager_GLX2 * m_ActiveManager;
    ...
    std::list<gMat44> m_Stack;

    gMat44 m_ProjectionMatrix;
    gMat44 m_ViewMatrix;
    gMat44 m_ModelMatrix;

    gMat44 m_ModelViewMatrix;
    gMat44 m_InvModelViewMatrix;

    gMat44 m_ProjectionViewMatrix;
    gMat44 m_ProjectionViewModelMatrix;

#ifdef NCK_GRAPH_RES_PROXY
    /// Texture Proxy Map
    std::map<std::string,ResourceProxy<Texture_GLX2>*> m_TextureProxys;

    ResourceProxy<Texture_GLX2> * GetTextureProxy(const std::string & fileName);
    ResourceProxy<Texture_GLX2> * AddTextureProxy(const std::string & fileName, Texture_GLX2 * tex);

    /**
    * Returns True if last texture object removed, GPU object can be safely released. 
     */
    bool RemoveTextureProxy(Texture_GLX2 * tex);

    /// Program Proxy Map
    std::map<std::string,ResourceProxy<Program_GLX2>*> m_ProgramProxys;

    ResourceProxy<Program_GLX2> * GetProgramProxy(const std::string & fileName);
    ResourceProxy<Program_GLX2> * AddProgramProxy(const std::string & fileName, Program_GLX2 * prog);

    /**
    * Returns True if last program object removed, GPU object can be safely released. 
     */
    bool RemoveProgramProxy(Program_GLX2 * prog);
#endif
    ...
    std::list<gMat44> m_ModelMatrixStack;
