...
Texture_GLX2::Texture_GLX2(Device_GLX2 *dev){
    m_Texture = GL_ZERO;
...
Texture_GLX2::~Texture_GLX2(){

}

Format Texture_GLX2::GetFormat(){
    return m_Format;
}

unsigned int Texture_GLX2::GetWidth(){
    return m_Width;
}

unsigned int Texture_GLX2::GetHeight(){
    return m_Height;
}

void Texture_GLX2::SetFilter(FilterType type,FilterMode mode)
{
...
void Texture_GLX2::SetAdressMode(AdressMode mode)
{
...
void Texture_GLX2::SetAnisotropyFilter(float value){
...
void Texture_GLX2::SetFilterAndWrapping()
{
...
GLenum Texture_GLX2::ConvertFilterMode(FilterMode mode){
    if(mode == FILTER_NONE)
        return GL_NONE;
...
void Texture_GLX2::GetGLPropertiesFromFormat(Format srcFormat, int * glComponents,
...
#ifdef NCK_GRAPH_RES_PROXY
ResourceProxy<Texture_GLX2> * Texture_GLX2::GetProxyManager(){
    return m_Proxy;
}
#endif 

Texture2D_GLX2::Texture2D_GLX2(Device_GLX2 *dev) : Texture_GLX2(dev){
    m_LockData = NULL;
    m_LockFlag = false;
    m_Target = GL_TEXTURE_2D;
    m_LastModified = 0;
...
TextureType Texture2D_GLX2::GetType(){
    return TEXTURE_2D;
}

void Texture2D_GLX2::Enable(unsigned int sampler_id)
{
...
void Texture2D_GLX2::Disable(unsigned int sampler_id)
{
    if(sampler_id >= 8)    return;
...
void *Texture2D_GLX2::Lock(unsigned int level)
{
    glEnable(m_Target);
    glBindTexture(m_Target,m_Texture);
...
void Texture2D_GLX2::Unlock(unsigned int level){
    glEnable(m_Target);
    glBindTexture(m_Target,m_Texture);
...
Texture2D_GLX2 * Texture2D_GLX2::Load(Device_GLX2 *dev,const std::string & filename, bool mipmaps){
    Texture2D_GLX2 * tex = new Texture2D_GLX2(dev);

#ifdef NCK_GRAPH_RES_PROXY
    ResourceProxy<Texture_GLX2> * proxy = NULL;
    Texture2D_GLX2 * refTex = NULL;

    if((proxy = dev->GetTextureProxy(filename)) != NULL && (refTex = dynamic_cast<Texture2D_GLX2*>(proxy->GetReference())) != NULL){
...
GLuint Texture2D_GLX2::getTextureId(){
    return m_Texture;
}

void Texture2D_GLX2::Create(unsigned int width, unsigned int height, Format format, bool renderTarget)
{
    m_Width = width;
    m_Height = height;
    m_Format = format;
...
TextureCubemap_GLX2::TextureCubemap_GLX2(Device_GLX2 *dev):Texture_GLX2(dev){
    m_Target = GL_TEXTURE_CUBE_MAP;
}

TextureCubemap_GLX2::~TextureCubemap_GLX2(){
    m_Device->m_TextureCache.Clear();
...
void TextureCubemap_GLX2::Create(unsigned int width, unsigned int height, Format format, bool renderTarget){
...
void TextureCubemap_GLX2::Enable(unsigned int sampler_id){
    if (sampler_id >= 8) return;
...
void TextureCubemap_GLX2::Disable(unsigned int sampler_id){
    if (sampler_id >= 8)return;
...
GLuint TextureCubemap_GLX2::getTextureId() {
    return m_Texture;
}

TextureType TextureCubemap_GLX2::GetType(){
    return TEXTURE_CUBEMAP;
}


void * TextureCubemap_GLX2::Lock(unsigned int face,unsigned int level){
    return NULL;
}

void TextureCubemap_GLX2::Unlock(unsigned int face,unsigned int level){

}

    
TextureCubemap_GLX2 * TextureCubemap_GLX2::Load(Device_GLX2 * dev, const std::string & packageFilename, bool mipmaps)
{
#ifdef NCK_GRAPH_RES_PROXY
    // Check texture proxy cache
    ResourceProxy<Texture_GLX2>  * proxy = NULL;
    TextureCubemap_GLX2 * refTex = NULL;

    if((proxy = dev->GetTextureProxy(packageFilename)) != NULL && (refTex = dynamic_cast<TextureCubemap_GLX2*>(proxy->GetReference())) != NULL)
    {
        TextureCubemap_GLX2 * ret = new TextureCubemap_GLX2(dev);
    ...
    TextureCubemap_GLX2 * ret = Load(dev,imgvec, mipmaps);
...
TextureCubemap_GLX2 * TextureCubemap_GLX2::Load(Device_GLX2 * dev, const std::vector<Core::Image*> & images, bool mipmaps){
...
RTManager_GLX2::RTManager_GLX2(Device_GLX2 *dev){
    m_Device = dev;
    m_RenderBuffer = 0;
    m_FrameBuffer = 0;
}

RTManager_GLX2::~RTManager_GLX2(){
    glDeleteFramebuffers(1,&m_FrameBuffer);
    glDeleteRenderbuffers(1,&m_RenderBuffer);
}

bool RTManager_GLX2::Enable(int face){
    ...
    for(std::list<AttachedTexture>::iterator i = m_Textures.begin();i!=m_Textures.end() && count < maxAttach;i++,count++){
        if (i->texture->GetType() == TEXTURE_CUBEMAP) {
            TextureCubemap_GLX2 * tex = dynamic_cast<TextureCubemap_GLX2*>(i->texture);
            ...
            Texture2D_GLX2 * tex = dynamic_cast<Texture2D_GLX2*>(i->texture);
...
GLuint RTManager_GLX2::GetWidth() {
    return m_Width;
}

GLuint RTManager_GLX2::GetHeight() {
    return m_Height;
}

void RTManager_GLX2::Load(GLuint width, GLuint height){
...
void RTManager_GLX2::DetachAll() {
    m_Textures.clear();
}

bool RTManager_GLX2::Attach(unsigned int target,Texture2D *tex){
    m_Textures.push_back(AttachedTexture(dynamic_cast<Texture_GLX2*>(tex),target,0));
    return true;
}

bool RTManager_GLX2::Attach(unsigned int target,TextureCubeMap *tex){
    m_Textures.push_back(AttachedTexture(dynamic_cast<TextureCubemap_GLX2*>(tex),target,0));
    return true;
}

bool RTManager_GLX2::Disable(){
...
bool RTManager_GLX2::InvertedY() {
    return true;
}

Texture3D_GLX2::Texture3D_GLX2(Device_GLX2 *dev) : Texture_GLX2(dev){
    m_Target = GL_TEXTURE_3D;
    m_Depth = 0;
}

Texture3D_GLX2::~Texture3D_GLX2(){
    m_Device->m_TextureCache.Clear();
...
void Texture3D_GLX2::Enable(unsigned int sampler_id){
    if(sampler_id >= 8)    
        return;
...
void Texture3D_GLX2::Disable(unsigned int sampler_id){
    if(sampler_id >= 8)    return;
...
TextureType Texture3D_GLX2::GetType(){
    return TEXTURE_3D;
}

GLuint Texture3D_GLX2::getTextureId(){
    return m_Texture;
}


Texture3D_GLX2 * Texture3D_GLX2::Load(Device_GLX2 * dev,const std::string & packageFilename, bool mipmaps)
{
#ifdef NCK_GRAPH_RES_PROXY
    ResourceProxy<Texture_GLX2> * proxy = NULL;
    Texture3D_GLX2 * refTex = NULL;

    if((proxy = dev->GetTextureProxy(packageFilename)) != NULL && (refTex = dynamic_cast<Texture3D_GLX2*>(proxy->GetReference())) != NULL){
        Texture3D_GLX2 * ret = new Texture3D_GLX2(dev);
        ret->m_Width = refTex->m_Width;
        ret->m_Height = refTex->m_Height;
        ret->m_Format = refTex->m_Format;
        ret->m_Target = refTex->m_Target;
    ...
    Texture3D_GLX2 * ret = Load(dev,imgvec, mipmaps);
...
Texture3D_GLX2 * Texture3D_GLX2::Load(Device_GLX2 * dev, const std::vector<std::string> & filenames, bool mipmaps)
{
    ...
    Texture3D_GLX2 * ret = Load(dev,images,mipmaps);
...
Texture3D_GLX2 * Texture3D_GLX2::Load(Device_GLX2 * dev, const std::vector<Core::Image*> & images, bool mipmaps)
{
    Core::Image * img = images[0];
...
unsigned int Texture3D_GLX2::GetDepth(){
    return m_Depth;
}
