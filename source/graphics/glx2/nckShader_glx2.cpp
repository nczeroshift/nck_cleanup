...
Program_GLX2::Program_GLX2(Device_GLX2 *dev){
    m_Device = dev;
    m_VertexShader = 0;
    m_FragmentShader = 0;
    m_Active = false;
...
Program_GLX2::~Program_GLX2()
{
    ...
    for(std::map<std::string,Shader_GLX2_Variable*>::iterator i = m_Variables.begin();i!=m_Variables.end();i++){
        SafeDelete(i->second);
    }
...
void Program_GLX2::Load(const std::string & source){
    ...
    // Load vertex shader
    {
        ...
        glShaderSource(vshd, (GLsizei)count+1, sources, NULL);
    ...
    if(fragment.size()>0)
    {
        ...
        glShaderSource(fshd, (GLsizei)count+1, sources, NULL);
        glCompileShader(fshd);

        int isCompiled;
        glGetShaderiv((GLsizei)count+1, GL_COMPILE_STATUS, &isCompiled);
...
void Program_GLX2::BindUniforms() {
    ...
    if (m_ModelNormalMatrix > -1) {
        gMat44 inv,trans;
        gInvMat44(inv, m_Device->m_ModelMatrix);
        gTransposeMat44(trans, inv);
    ...
    if (m_Time > -1)
        glUniform1f(m_Time, (float)GetGlobalTime());
...
void Program_GLX2::LoadFromFilename(const std::string & filename)
{
#ifdef NCK_GRAPH_RES_PROXY
    ResourceProxy<Program_GLX2>  * proxy = NULL;
    Program_GLX2 * refProg = NULL;
...
void Program_GLX2::EnableInternal(){
    if(!m_Active){
...
void Program_GLX2::Disable(){
    m_ToRemove = true;
}

void Program_GLX2::DisableInternal(){
    m_ToRemove = false;
    m_Active = false;
...
Shader_GLX2_Variable * entryGetter(const std::map<std::string,Shader_GLX2_Variable*> & map, const std::string & key)
{
    std::map<std::string,Shader_GLX2_Variable*>::const_iterator it = map.find(key);
...
int Program_GLX2::GetVariableId(const std::string & name){
    return glGetUniformLocation(m_Program,name.c_str());
}

bool Program_GLX2::SetVariable1i(const std::string & name,int value){

    Shader_GLX2_Variable *var = entryGetter(m_Variables,name);
    if(!var){
        ...
        {
            var = new Shader_GLX2_Variable();
            var->m_Name = name;
            var->m_Handle = id;
            var->m_Type = Shader_GLX2_Variable::INT1;
            var->m_Count = 1;
            var->m_Data = new int;
            *((int*)var->m_Data) = value;
            m_Variables.insert(std::pair<std::string,Shader_GLX2_Variable*>(name,var));
        }
    }
    *((int*)var->m_Data) = value;
    return true;
...
bool Program_GLX2::SetVariable1f(const std::string & name,float value){
    Shader_GLX2_Variable *var = entryGetter(m_Variables,name);
    if(!var){
        GLint id = glGetUniformLocation(m_Program,name.c_str());
        if(id==-1)
        ...
        else
        {
            ...
            var->m_Name = name;
            var->m_Handle = id;
            var->m_Type = Shader_GLX2_Variable::FLOAT1;
            var->m_Count = 1;
...
bool Program_GLX2::SetVariable4i(const std::string & name,int x,int y,int z,int w){
    Shader_GLX2_Variable *var = entryGetter(m_Variables,name);
    if(!var){
        GLint id = glGetUniformLocation(m_Program,name.c_str());
        if(id==-1)
        ...
        else
        {
            ...
            var->m_Name = name;
            var->m_Handle = id;
            var->m_Type = Shader_GLX2_Variable::INT4;
            var->m_Count = 1;
...
bool Program_GLX2::SetVariable4f(const std::string & name,float x,float y,float z,float w){
    Shader_GLX2_Variable *var = entryGetter(m_Variables,name);
    if(!var){
        GLint id = glGetUniformLocation(m_Program,name.c_str());
        if(id==-1)
        ...
        else
        {
            ...
            var->m_Name = name;
            var->m_Handle = id;
            var->m_Type = Shader_GLX2_Variable::FLOAT4;
            var->m_Count = 1;
...
bool Program_GLX2::SetArray1f(const std::string & name,int count,const float *value){
    Shader_GLX2_Variable *var = entryGetter(m_Variables,name);
    if(!var){
        GLint id = glGetUniformLocation(m_Program,name.c_str());
        if(id==-1)
        ...
        else
        {
            ...
            var->m_Name = name;
            var->m_Handle = id;
            var->m_Type = Shader_GLX2_Variable::FLOAT1_ARRAY;
            var->m_Count = count;
...
bool Program_GLX2::SetArray4f(const std::string & name,int count,const float *value){
    Shader_GLX2_Variable *var = entryGetter(m_Variables,name);
    if(!var){
        GLint id = glGetUniformLocation(m_Program,name.c_str());
        if(id==-1)
        ...
        else
        {
            ...
            var->m_Name = name;
            var->m_Handle = id;
            var->m_Type = Shader_GLX2_Variable::FLOAT4_ARRAY;
            var->m_Count = count;
...
bool Program_GLX2::SetMatrix(const std::string & name,const float *mat){
    Shader_GLX2_Variable *var = entryGetter(m_Variables,name);
    if(!var){
        GLint id = glGetUniformLocation(m_Program,name.c_str());
        if(id==-1)
        ...
        else
        {
            ...
            var->m_Name = name;
            var->m_Handle = id;
            var->m_Type = Shader_GLX2_Variable::MATRIX;
            var->m_Count = 1;
...
bool Program_GLX2::SetMatrixArray(const std::string & name, int count,const float *v){
    Shader_GLX2_Variable *var = entryGetter(m_Variables,name);
    if(!var){
        GLint id = glGetUniformLocation(m_Program,name.c_str());
        if(id==-1)
        ...
        else
        {
            ...
            var->m_Name = name;
            var->m_Handle = id;
            var->m_Type = Shader_GLX2_Variable::MATRIX_ARRAY;
            var->m_Count = count;
...
void Shader_GLX2_Variable::Bind()
{
    switch(m_Type)
    {
    case FLOAT1:
...
ResourceProxy<Program_GLX2> * Program_GLX2::GetProxyManager(){
    return m_Proxy;
}
