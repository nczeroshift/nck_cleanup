...
Mesh_GLX2::Mesh_GLX2(Device_GLX2 *dev){
    m_Device = dev;
    m_VertexBuffer = 0;
    m_VerticeCount = 0;
}

void Mesh_GLX2::Load(uint32_t vertices,VertexProfile profile, void * vertexdata,
...
void Mesh_GLX2::Render(unsigned int subset)
{
    if (subset >= m_IndexBuffer.size())
        return;
...
unsigned int Mesh_GLX2::GetSubSetCount(){
    return (unsigned int)m_IndexBuffer.size();
}

unsigned int Mesh_GLX2::GetVerticesCount(){
    return m_VerticeCount;
}

unsigned int Mesh_GLX2::GetFacesCount()
{
    GLuint totalFaces = 0;
...
void Mesh_GLX2::GetVertexProfile(VertexProfile *p){
    *p = m_VertexProfile;
}

DynamicMesh_GLX2::DynamicMesh_GLX2(Device_GLX2 *dev){
    m_Device = dev;
    m_MaxVertices = 0;
    m_IndexBuffer = 0;
}

DynamicMesh_GLX2::~DynamicMesh_GLX2(){
    SafeArrayDelete(m_VertexBuffer);
    SafeArrayDelete(m_IndexBuffer);
}

void DynamicMesh_GLX2::Create(int maxVertices, int maxTriangles){
...
void DynamicMesh_GLX2::Disable(){
    glDisableClientState (GL_NORMAL_ARRAY);
    glDisableClientState (GL_VERTEX_ARRAY);
    glDisableClientState (GL_TEXTURE_COORD_ARRAY);
}

void DynamicMesh_GLX2::Render(){
    glDrawElements(GL_TRIANGLES,m_TrianglesCount*3,GL_UNSIGNED_INT,(void*)m_IndexBuffer);
}

void DynamicMesh_GLX2::LockVBO(void ** data){
    *data = (void*)m_VertexBuffer;
}

void DynamicMesh_GLX2::UnlockVBO(){

}

void DynamicMesh_GLX2::LockIBO(void ** data){
    *data = (void*)m_IndexBuffer;
}

void DynamicMesh_GLX2::UnlockIBO(){

}

unsigned int DynamicMesh_GLX2::GetMaxVertices(){
    return m_MaxVertices;
}

unsigned int DynamicMesh_GLX2::GetMaxTriangles(){
    return m_MaxTriangles;
}

unsigned int DynamicMesh_GLX2::GetVerticesCount(){
    return m_VerticesCount;
}

unsigned int DynamicMesh_GLX2::GetTrianglesCount(){
    return m_TrianglesCount;
}

void DynamicMesh_GLX2::SetVerticesCount(unsigned int count){
    m_VerticesCount = count;
}

void DynamicMesh_GLX2::SetTrianglesCount(unsigned int count){
