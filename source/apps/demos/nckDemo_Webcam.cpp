...
void Demo_Webcam::Render(float dt){
    ...
    if (targetData != NULL && changed) {
        ...
        for (int y = 0; y < videoTexture->GetHeight(); y++) {
            for (int x = 0; x < videoTexture->GetWidth(); x++) {
                int pos = y * videoTexture->GetWidth() * 3 + x * 3;

                uint8_t b = ((uint8_t*)targetData)[pos + 2];
                uint8_t g = ((uint8_t*)targetData)[pos + 1];
                uint8_t r = ((uint8_t*)targetData)[pos];

                ((uint8_t*)data)[pos] = b;
                ((uint8_t*)data)[pos + 1] = g;
                ((uint8_t*)data)[pos + 2] = r;
            }
        }
