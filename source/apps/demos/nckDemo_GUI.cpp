...
Demo_GUI::Demo_GUI(Core::Window * wnd, Graph::Device * dev){
    this->dev = dev;
    this->wnd = wnd;
...
void Demo_GUI::Load(){
    ...
    wgtRender = new Gui::BlenderWidgetRenderer(dev, shpRender, font);
...
void Demo_GUI::UpdateWndEvents(){
    Gui::UserInputState state;
    state.m_CursorX = wnd->GetMousePosition().GetX();
    state.m_CursorY = wnd->GetMousePosition().GetY();
    state.m_State_LMB = (wnd->GetMouseButtonStatus(Core::MOUSE_BUTTON_LEFT) != Core::BUTTON_STATUS_UP);
    wgtManager->Update(state);
}

std::vector<std::string> Demo_GUI::GetKeywords() {
