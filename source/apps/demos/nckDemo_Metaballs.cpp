...
void Demo_Metaballs::Render(float dt){
    ...
    font->Draw(10, 20, std::string("Triangles: ") + Math::IntToString(processor->GetTrianglesCount()));
    font->Draw(10, 44, std::string("Vertices: ") + Math::IntToString(processor->GetVerticesCount()));
    font->Draw(10, 68, std::string("Lod: ") + Math::IntToString(processor->GetDetail()) + " Max Lod: " + Math::IntToString(processor->GetMaxDetail()));
    font->Draw(10, 92, std::string("FPS: ") + Math::FloatToString(fps, 2));
...
void Demo_Metaballs::UpdateWndEvents(){
    ...
    if (wnd->GetKeyStatus(Core::KEY_UP) == Core::BUTTON_STATUS_DOWN) {
        if (!statPlusBtn)
    ...
    if (wnd->GetKeyStatus(Core::KEY_DOWN) == Core::BUTTON_STATUS_DOWN) {
        if (!statMinusBtn)
            currentLod = processor->GetDetail() - 1;
        statMinusBtn = true;
    }
