...
void Demo_Serial::UpdateWndEvents(){
    ...
    if (!btnStatusC && wnd->GetKeyStatus(Core::KEY_C) == Core::BUTTON_STATUS_DOWN) {
        const char * simpleString = "foo\n\r";
        size_t size = strlen(simpleString);
        serial->SendData((unsigned char*)simpleString, (unsigned int)size);
    }
    btnStatusC = wnd->GetKeyStatus(Core::KEY_C) == Core::BUTTON_STATUS_DOWN;

    if (!btnStatusZ && wnd->GetKeyStatus(Core::KEY_Z) == Core::BUTTON_STATUS_DOWN) {
        const char * simpleString = "bar\n\r";
        size_t size = strlen(simpleString);
        serial->SendData((unsigned char*)simpleString, (unsigned int)size);
    }
    btnStatusZ = wnd->GetKeyStatus(Core::KEY_Z) == Core::BUTTON_STATUS_DOWN;
}
