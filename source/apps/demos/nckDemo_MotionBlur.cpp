...
#include "nckDemo_MotionBlur.h"

Demo_MotionBlur::Demo_MotionBlur(Core::Window * wnd, Graph::Device * dev){
    this->dev = dev;
    this->wnd = wnd;
    time = 0;
}

Demo_MotionBlur::~Demo_MotionBlur(){
}

void Demo_MotionBlur::Load(){
    // Enable z test
...
void Demo_MotionBlur::Render(float dt){
...
void Demo_MotionBlur::UpdateWndEvents(){
    
}

std::vector<std::string> Demo_MotionBlur::GetKeywords() {
...
std::string Demo_MotionBlur::GetDescription() {
    return "Motion Blur";
}

Demo * CreateDemo_MotionBlur(Core::Window * wnd, Graph::Device * dev){
    return new Demo_MotionBlur(wnd,dev);
}
