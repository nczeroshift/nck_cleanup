...
__m128 Mat44::_pSSE_LinComb(const __m128 & a, const Math::Mat44 & B)
{
    __m128 result;
    result = _mm_mul_ps(_mm_shuffle_ps(a, a, 0x00), B.m_r[0]);
    result = _mm_add_ps(result, _mm_mul_ps(_mm_shuffle_ps(a, a, 0x55), B.m_r[1]));
    result = _mm_add_ps(result, _mm_mul_ps(_mm_shuffle_ps(a, a, 0xaa), B.m_r[2]));
    result = _mm_add_ps(result, _mm_mul_ps(_mm_shuffle_ps(a, a, 0xff), B.m_r[3]));
    return result;
}
...
Mat44 operator * (const Mat44 & matA, const Mat44 & matB){
    Mat44 res;
    #ifdef NCK_OPTIM_SSE
    // Fabian 'ryg' Giesen https://gist.github.com/rygorous/4172889
    // out_ij = sum_k a_ik b_kj
    // => out_0j = a_00 * b_0j + a_01 * b_1j + a_02 * b_2j + a_03 * b_3j
    __m128 out0x = Mat44::_pSSE_LinComb(matA.m_r[0], matB);
    __m128 out1x = Mat44::_pSSE_LinComb(matA.m_r[1], matB);
    __m128 out2x = Mat44::_pSSE_LinComb(matA.m_r[2], matB);
    __m128 out3x = Mat44::_pSSE_LinComb(matA.m_r[3], matB);
...
Mat44 RotateX(const float & alpha)
{
    ...
    res.m_n[1][1] = cosf(alpha);
    res.m_n[1][2] = sinf(alpha);
    res.m_n[2][1] = -sinf(alpha);
    res.m_n[2][2] = cosf(alpha);
...
{
    Mat44 res = Identity();
    res.m_n[0][0] = cosf(alpha);
    res.m_n[0][2] = -sinf(alpha);
    res.m_n[2][2] = cosf(alpha);
    res.m_n[2][0] = sinf(alpha);
    return res;
}
...
Mat44 RotateZ(const float & alpha)
{
    Mat44 res = Identity();
    res.m_n[0][0] = cosf(alpha);
    res.m_n[0][1] = sinf(alpha);
    res.m_n[1][0] = -sinf(alpha);
    res.m_n[1][1] = cosf(alpha);
    return res;
}
