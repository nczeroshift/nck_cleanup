...
QueueBuffer::QueueBuffer(uint64_t maxCapacity){
...
uint64_t QueueBuffer::Push(void * data, uint64_t size){
...
uint64_t QueueBuffer::Push(const std::string & str) {
...
uint64_t QueueBuffer::Size(){
    return m_Size;
}

uint64_t QueueBuffer::Pop(void * targetData, uint64_t targetSize){
    uint64_t ret = MIN(targetSize,m_Size);
...
uint64_t QueueBuffer::ToString(std::string * str) {
