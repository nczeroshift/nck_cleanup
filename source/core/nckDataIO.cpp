...
FileReader * FileReader::Open(const std::string & filename){
    ...
    FILE * temp = NULL;
    fopen_s(&temp, resolved.c_str(), "rb");
...
size_t FileReader::Read(void * data, size_t size){
    size_t elements = fread(data, 1, size, fHandle);
    return elements;
}
...
bool FileReader::Exists(const std::string & filename){
    FILE * f = NULL;
    fopen_s(&f, ResolveFilename(filename).c_str(), "rb");
...
uint64_t FileReader::Size(const std::string & filename){
#if defined(NCK_WINDOWS)
     struct __stat64 buf;
...
MemoryReader::MemoryReader(uint8_t *data, uint64_t size, bool copy){
    if(copy){
...
FileWriter * FileWriter::Open(const std::string & filename, bool append) {
    ...
    std::string resolved = ResolveFilename(filename);
    FILE * file = NULL;
    fopen_s(&file, resolved.c_str(), "wb");
